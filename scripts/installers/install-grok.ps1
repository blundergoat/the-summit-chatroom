#!/usr/bin/env pwsh
# GOAT System Installer - Grok CLI (PowerShell)
...
Write-Host "Starting Grok CLI installation process..." -ForegroundColor Cyan
Write-Host "This will install Grok CLI from Vibe Kit" -ForegroundColor Yellow
...
if (Test-Command node) {
    ...
    Write-Host "Node.js is required for Grok CLI." -ForegroundColor Red
    $installNode = Read-Host "Would you like to install Node.js? (y/n)"
    if ($installNode -ne "y") {
        Write-Host "Node.js is required for Grok CLI. Exiting." -ForegroundColor Red
        exit 1
    }
...
Write-Host "`nInstalling Grok CLI via npm..." -ForegroundColor Cyan
npm install -g @vibe-kit/grok-cli
if ($LASTEXITCODE -ne 0) {
    Write-Host "Error installing Grok CLI." -ForegroundColor Red
    Write-Host "Troubleshooting steps:" -ForegroundColor Yellow
    Write-Host "1) Check internet connection" -ForegroundColor White
    Write-Host "2) npm config list" -ForegroundColor White
    Write-Host "3) Try: npm install -g @vibe-kit/grok-cli" -ForegroundColor White
    exit 1
}

Write-Host "`nVerifying installation..." -ForegroundColor Yellow
if (Test-Command grok) {
    Write-Host "Grok CLI installed successfully!" -ForegroundColor Green
    grok --version 2>$null
    Ensure-NpmGlobalPath
    Show-PathPrefixWarning

    Write-Host "`nSetting up Grok API key" -ForegroundColor Cyan
    $grokKey = Read-Host "Enter your Grok API key (leave empty to skip)"
    if ($grokKey) {
        $grokDir = Join-Path $HOME ".grok"
        New-Item -ItemType Directory -Force -Path $grokDir | Out-Null
        $configPath = Join-Path $grokDir "user-settings.json"
        @{ apiKey = $grokKey } | ConvertTo-Json | Set-Content -Path $configPath
        Write-Host "Saved API key to $configPath" -ForegroundColor Green
    } else {
        Write-Host "Skipped API key setup. You can add it later in ~/.grok/user-settings.json" -ForegroundColor Yellow
    }

    Write-Host "`nNext steps:" -ForegroundColor Cyan
    Write-Host "1) Start the CLI: grok" -ForegroundColor White
    Write-Host "2) Use 'grok --help' to see commands" -ForegroundColor White
} else {
    Write-Host "Grok CLI installed but command not found in PATH. Restart your terminal or add the npm global bin to PATH (npm config get prefix)." -ForegroundColor Yellow
}
